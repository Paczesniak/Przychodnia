...
<head>
    ...
    <title>Przychodnia - Lekarze</title>
...
<body>
    ...
    <main>
        <section id="lekarze" class="lekarze py-5">
            <div class="container">
                <h2 class="section-title mt-2">Lekarze</h2>
                <div class="btn mb-3">
                    <a href="{{ route('lekarze.create') }}" class="btn btn-outline-primary">Dodaj Lekarza</a>
                </div>
                    ...
                    <table class="table table-striped text-white mt-5 mb-4">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $lekarz->user->email }}</td>
                                <td>{{ $lekarz->telefon }}</td>
                                <td>
                                    <form action="{{ route('lekarze.destroy', $lekarz->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger">Usuń</button>
                                    </form>
                                    <a href="{{ route('lekarze.show', $lekarz->id) }}" class="btn btn-sm btn-outline-primary">Informacje</a>
                                </td>
                                <td>
                                    <a href="{{ route('lekarze.visits_for_day', ['id' => $lekarz->id, 'date' => now()->toDateString()]) }}" class="btn btn-outline-warning">Wizyty dzisiaj</a>
                                    <form action="{{ route('lekarze.visits_for_day', ['id' => $lekarz->id]) }}" method="get" style="display:inline-block;">
                                        <input type="date" name="date" value="{{ now()->toDateString() }}" required>
                                        <button type="submit" class="btn btn-outline-success">Wizyty wg daty</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
