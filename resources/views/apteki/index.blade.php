...
<body>
    ...
    <main>
        <section id="apteki" class="apteki py-5">
            <div class="container">
                <h2 class="section-title mt-2">Lista Aptek</h2>
                <div class="btn mb-3">
                    <a href="{{ route('apteki.create') }}" class="btn btn-outline-primary">Dodaj Aptekę</a>
                </div>
                    ...
                    <table class="table table-striped text-white mt-5 mb-4">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $apteka->ulica }}</td>
                                <td>{{ $apteka->numer }}</td>
                                <td>{{ $apteka->telefon }}</td>
                                <td>{{ $apteka->email }}</td>
                                <td>
                                    <a href="{{ route('apteki.edit', $apteka->id) }}" class="btn btn-sm btn-outline-warning">Edytuj</a>
                                    <form action="{{ route('apteki.destroy', $apteka->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
