...
<body>
    ...
    <main>
        <section id="wizyty" class="wizyty py-5">
            <div class="container">
                <h2 class="section-title mt-2">Historia Wizyt</h2>
                <form method="GET" action="{{ route('historia.index') }}">
                    <div class="form-row mb-4">
                        <div class="col">
                            <input type="date" style="background-color:rgb(65, 77, 78); border-color:#f96161; color: white;" class="form-control" name="data_od" placeholder="Data od" value="{{ request('data_od') }}">
                        </div>
                        <div class="col">
                            <input type="date" style="background-color:rgb(65, 77, 78);  border-color:#f96161; color: white;" class="form-control" name="data_do" placeholder="Data do" value="{{ request('data_do') }}">
                        </div>
                        <div class="col">
                            <select style="background-color:rgb(65, 77, 78);  border-color:#f96161; color: white;" class="form-control " name="status">
                                <option value="">Wszystkie</option>
                                <option value="aktywny" {{ request('status') == 'aktywny' ? 'selected' : '' }}>Aktywne</option>
                                <option value="usunięte" {{ request('status') == 'usunięte' ? 'selected' : '' }}>Usunięte</option>
                            </select>
                        </div>
                        <div class="col">
                            <button type="submit" class="btn btn-outline-primary">Filtruj</button>
                            <a href="{{ route('historia.index') }}" class="btn btn-outline-danger">Wyczyść</a>
                        </div>
                    </div>
                </form>
                ...
                <div class="table-responsive">
                    <table class="table table-striped text-white mt-5 mb-4">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ strpos($wizyta->opis, 'Wizyta została usunięta') !== false ? 'deleted-visit' : '' }}">
                                <td>{{ $wizyta->id }}</td>
                                <td>{{ $wizyta->pacjent_imie }} {{ $wizyta->pacjent_nazwisko }}</td>
                                <td>{{ $wizyta->lekarz_imie }} {{ $wizyta->lekarz_nazwisko }}</td>
                                <td>{{ $wizyta->opis }}</td>
                                <td>{{ $wizyta->choroba ?? 'Brak' }}</td>
                                <td>{{ $wizyta->leczenie ?? 'Brak' }}</td>
