...
<body>
    ...
    <main>
        <section id="wizyty" class="wizyty py-5">
            <div class="container">
                <h2 class="section-title mt-2">Wizyty</h2>
                <div class="btn mb-3">
                    <a href="{{ route('wizyty.create') }}" class="btn btn-outline-primary">Dodaj Wizytę</a>
                </div>
                <div class="table-responsive">
                    <table class="table table-striped text-white mt-5 mb-4">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $wizyta->id }}</td>
                                <td>{{ $wizyta->pacjent->imie }} {{ $wizyta->pacjent->nazwisko }}</td>
                                <td>{{ $wizyta->lekarz->imie }} {{ $wizyta->lekarz->nazwisko }}</td>
                                <td>{{ $wizyta->opis }}</td>
                                <td>{{ $wizyta->choroba ? $wizyta->choroba->nazwa : 'Brak' }}</td>
                                <td>{{ $wizyta->leczenie ? $wizyta->leczenie->nazwa : 'Brak' }}</td>
                                <td>{{ $wizyta->data_wizyty }}</td>
                                <td>
                                    <a href="{{ route('wizyty.edit', $wizyta->id) }}" class="btn btn-sm btn-outline-warning">Edytuj</a>
                                    <form action="{{ route('wizyty.destroy', $wizyta->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-danger">Usuń</button>
                                    </form>
                                    <a href="{{ route('wizyty.show', $wizyta->id) }}" class="btn btn-sm btn-outline-primary">Informacje</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
